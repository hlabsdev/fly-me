defmodule Fly.Client do
  ...
  # Hlabs: used in AppLive.Index.fetch_apps
  # Get my apps list fron the GraphQl API
  ...
  # Hlabs: used in AppLive.Show.fetch_app
  # Get an app's details fron the GraphQl API
  ...
  # Hlabs: used in AppLive.Show.fetch_appstatus
  # Get an app's status fron the GraphQl API with the showCompleted set to true
  def fetch_appstatus(name, config) do
    """
      query($name: String!) {
    ...
    """
    |> perform_query(%{name: name}, config, :fetch_appstatus)
    |> handle_response()
    |> case do
      {:ok, %{"appstatus" => status}} ->
        Logger.info("appstatus returned: #{inspect(status)}")
        {:ok, status}
      ...
      other ->
        Logger.error("Unexpected result from fetch_appstatus. Response: #{inspect(other)}")

        {:error, "Failed to fetch app's status"}
    end
  end

  # Hlabs: used in SessionController.create
  ...
  # Hlabs: If user is unauthenticated show error
  ...
  # Hlabs: Handles success response
  defp handle_http_response({:ok, %HTTPoison.Response{status_code: 200}}) do
    :ready
  end

  # Hlabs: Handles responses only when status code is between 300 or 399 (redirection code)
  defp handle_http_response({:ok, %HTTPoison.Response{status_code: redirect}})
       when redirect in 300..399 do
    :ready
  end

  # Hlabs: Handle not found response
  defp handle_http_response({:ok, %HTTPoison.Response{status_code: 404}}) do
    :not_ready
  end

  # Hlabs: Handle client side error response only when status code is between 400 or 499
  ...
  # Hlabs: Handle server side error response only when status code is between 500 or 599
